...
- 输入：通过Python读入文件`pro2_in1.txt`（该文件可在`Course-Projects/`下找到）得到数据。文件中每一行代表一组输入数据。比如第一行为：`[-4, -1, 0, 3, 10]`，数字间用逗号隔开，一组数据由一个中括号括起来。
...
- 输入：通过Python读入文件`pro2_in2.txt`（该文件可在`Course-Projects/`下找到）得到数据。文件中每一行代表一个输入矩阵，矩阵每个元素用一个空格隔开，矩阵每行用一个逗号隔开。比如第一行为：`o o o ，o o o ,o o o `，代表一个3x3的由`o`组成的矩阵。
